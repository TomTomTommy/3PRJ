...
public class tme {
	...
	public static Mat WriteRec(Mat idft, Mat src) {
		MinMaxLocResult max = Core.minMaxLoc(idft);
		double x = max.maxLoc.x;
		double y = max.maxLoc.y;
		Imgproc.rectangle(src, new Point(x-facewidth/2, y-faceheight/2), new Point(x+facewidth/2, y+faceheight/2), new Scalar(0, 0, 255), 2);
		return src;
	}
	...
	public static void tme() throws Exception {
